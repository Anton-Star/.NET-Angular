module app.modal.templates {
    ...
    class MultiviewAnswersController {
        isBusy = false;
        ...
        onRadioSelected(index, sectionIndex, question: services.IQuestion, answer: services.IAnswer): void {
            ...
            if (thisQuestion) {
                _.each(thisQuestion.answers, (a) => {
                    a.selected = a.id === answer.id;
                });

                answer.selected = true;

                this.checkAnswers(index, thisQuestion);
            }
        }

        onCheckboxSelected(index, sectionIndex, question: services.IQuestion): void {
            if (sectionIndex === 0) this.checkAnswers(index, question);
        }

        onSetOtherText(index, sectionIndex, answer, responseIndex) {            
            responseIndex = responseIndex || 0;
            if (sectionIndex > 0) return;

            _.each(this.sections, (siteSection,i) => {
        ...
        save(isAutoSave: boolean): void {
            ...
            this.isBusy = true;
            this.applicationService.saveMultiviewSection(this.organization, this.appType, sections)
        ...
        onFromDateChanged(index: number, sectionIndex, q: services.IQuestion, questionResponse: services.IQuestionResponse, responseIndex?: number): void {
            responseIndex = responseIndex || 0;
        ...
        onToDateChanged(index: number, sectionIndex, q: services.IQuestion, questionResponse: services.IQuestionResponse, responseIndex?: number): void {
        ...
        onUserChange(index: number, sectionIndex, value: string, responseIndex?: number) {
            responseIndex = responseIndex || 0;
