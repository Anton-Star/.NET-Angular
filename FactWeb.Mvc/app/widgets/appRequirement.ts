...
module app.widgets {
    ...
    class AppRequirement implements ng.IDirective {
        ...
        static factory(): ng.IDirectiveFactory {
            var directive = (
                ...
                new AppRequirement($window, $rootScope, $timeout, $compile, $templateRequest, $uibModal, config, common, modalHelper, notificationFactory, requirementService, questionService);
        ...
        link = (scope: any, element: any, attributes: any) => {
            ...
            if (angular.isArray(scope.row.children) && scope.row.children.length > 0) {
                this.$templateRequest("/app/widgets/appRequirementChildren.html").then((html) => {
                    var template = angular.element(html);
                    element.append(template);
                    this.$compile(template)(scope);
                });
            } else {
                this.$templateRequest("/app/widgets/appRequirement.html").then((html) => {
                    var template = angular.element(html);
                    element.append(template);
                    this.$compile(template)(scope);
                });
    ...
    angular
        .module('app.widgets')
        .directive('appRequirement', AppRequirement.factory());
}